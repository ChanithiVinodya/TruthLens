...
const ConfidenceRing = ({ confidence = 0, label, color }) => {
  ...
  // Use provided color when supplied by parent card; fallback keeps prior behavior.
  let progressColor = color || theme.danger;
  if (!color) {
    if (safeConfidence >= 75) progressColor = theme.primary;
    else if (safeConfidence >= 40) progressColor = theme.warning;
  }
