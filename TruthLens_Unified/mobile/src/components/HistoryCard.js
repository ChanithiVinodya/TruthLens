...
export default function HistoryCard({ item, onPress, onDelete, onReanalyse }) {
  ...
  const getRiskMeta = (value) => {
    if (value >= 70) return { label: 'LOW RISK', color: '#00C896' };
    if (value >= 40) return { label: 'MEDIUM RISK', color: '#F5A623' };
    return { label: 'HIGH RISK', color: '#E05252' };
  };

  const riskMeta = getRiskMeta(normalizedScore);
  const riskColor = riskMeta.color;
        ...
        <View style={styles.topRow}>
          ...
          <View style={styles.rightSection}>
            ...
            <View style={[styles.riskBadge, { backgroundColor: `${riskColor}1A` }]}>
              <Text style={[styles.riskBadgeText, { color: riskColor }]}>● {riskMeta.label}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={[styles.scoringSection, { backgroundColor: riskColor + '10', borderColor: riskColor + '30' }]}>
          <View style={styles.scoringInfo}>
            ...
            <View style={styles.scoreBadgeRow}>
                ...
                <View style={[styles.levelBadge, { backgroundColor: `${riskColor}1A`, borderColor: `${riskColor}40` }]}>
                   <Text style={[styles.levelBadgeText, { color: riskColor }]}>● {riskMeta.label}</Text>
                </View>
            </View>
          </View>
